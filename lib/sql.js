...
var mysqlTest = {
    ...
    getRecord: function(param,res){
        ...
        pool.getConnection(function(err, connection) {    
            ...
            connection.query(sql, function(err, rows) {
                onErr(err,d)
                var newSql = db.recording.selectNo + rows.cardNo
    ...
    //     pool.getConnection(function(err, connection) {
    //         onErr(err,d)
    //         var sql = db.select+' LIMIT '+connection.escape(parseInt(params.pageNum*10))+','+connection.escape(parseInt(params.pageSize))
    //         connection.query(sql,function(err, rows){
    //             onErr(err,d)
